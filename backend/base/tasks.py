...
def postprocess_markdown_output(path_to_markdown:Union[str, Path])->None:
    with open(path_to_markdown, 'r+') as f:
        ...
        for line in f:
            ...
            # table processing
            line = re.sub(r'(?:\\tag)|(?:\\cline\{\d+\-\d+\})', ' ', line)
            line = re.sub(r'(?:\\begin{table})|(?:\\end{table})', '', line)
            if line.startswith(r'\begin{tabular}'):
                line = re.sub('tabular', 'array', line)
                line = re.sub(r'(?:multicolumn\{(\d)*\}\{[|c\*]+\})|(?:multiline\{(\d)*\}\{[|c\*]+\})|(?:multirow\{(\d)*\}\{[|c\*]+\})', 'text', line)
            line = re.sub(r'(?<=(?:\\begin\{))split', 'align', line)
            line = re.sub(r'(?<=(?:\\end\{))split', 'align', line)
            line = re.sub(r'mbox', 'text', line)
